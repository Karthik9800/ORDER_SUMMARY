...
const x=()=>{
 ...
 const validatePasswords = () => {
    ...
    for (let i = 0; i < passwords.length; i++) {
      ...
      for (let j = i; j < passwords.length; j++) {
        
        if (passwords[i].length < 8 || passwords[j].length < 8) {
          Alert.alert("Error", "Both passwords must be at least 8 characters.");
          return;
        }
        
        if (i === 0 && j === 1 && passwords[i] !== passwords[j]) {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.tabRow}>
        <Text style={styles.tabText} onPress={() => navigation.navigate('SignIn Page')}>sign in</Text>
        <Text
      ...
      <TouchableOpacity style={styles.loginBtn} onPress={validatePasswords}>
